...
public class AccountService implements UserDetailsService {
    ...
    private void sendSignUpConfirmEmail(Account newAccount) {
        ...
        context.setVariable("linkName", "스터디올레, 회원 가입 인증");
        context.setVariable("host", appProperties.getHost());
        context.setVariable("link", "/check-email-token?token=" + newAccount.getEmailCheckToken() +
        "&email=" + newAccount.getEmail());
        context.setVariable("message", "회원가입 완료를 위해 인증해주세요.");
    ...
    public void sendLoginWithoutPasswordEmail(String email) {
        ...
        context.setVariable("linkName", "스터디올레, 이메일로 로그인");
        context.setVariable("host", appProperties.getHost());
        context.setVariable("link", "/email-login-token?token=" + byEmail.getEmailLoginToken() +
                "&email=" + byEmail.getEmail());
        context.setVariable("message", "이메일로 로그인하려면 링크를 클릭해주세요.");

        String message = templateEngine.process("mail/simple-link", context);

        EmailForm emailForm = EmailForm.builder()
                .to(email)
                .subject("스터디올레, 이메일로 로그인")
                .message(message)
                .build();
